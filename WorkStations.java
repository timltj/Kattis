        ...
        while (!pqR.isEmpty()) {
            ...
            for (int x = 0; x < pqWS.size() + 1; x++) {
                if (pqWS.isEmpty()) { // first researcher
                    ...
                    if (pqR.peek().arrTime <= curVacancyTime) { // arrival when workstations are in use
                        pqWS.add(doneTime);
                        pqWS.add(curVacancyTime); // add new workstation
                        pqR.poll();
                        break;
                    }                    
                    if (pqR.peek().arrTime > curVacancyTime &&
                            pqR.peek().arrTime <= curVacancyTime + m + 1) { // arrival when at least 1 workstation is vacant
                        pqWS.add(doneTime);
                        pqR.poll();
                        count++;                        
                        break;
